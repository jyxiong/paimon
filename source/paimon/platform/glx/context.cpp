...
bool GlxContext::makeCurrent() {
    auto* display = Platform::instance()->display();
    const auto success = glXMakeContextCurrent(display, m_drawable, m_drawable, m_contextHandle);
    if (!success) {
...
void GlxContext::createContext(GLXContext shared,
    ...
    m_contextHandle = glXCreateContextAttribsARB(display, fbConfig[0], shared, True, contextAttributes.data());
    XSync(display, false);
    ...
    auto successLoad = gladLoaderLoadGL();
    if (!successLoad) {
        glXDestroyContext(display, m_contextHandle);
        m_contextHandle = nullptr;
