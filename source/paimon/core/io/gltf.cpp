...
template <>
void GltfLoader::ExtractAccessorData<glm::vec3>(const tinygltf::Model& model,
                                                int accessor_index,
                                                std::vector<glm::vec3>& out_data);

template <>
void GltfLoader::ExtractAccessorData<glm::vec2>(const tinygltf::Model& model,
                                                int accessor_index,
                                                std::vector<glm::vec2>& out_data);

template <>
void GltfLoader::ExtractAccessorData<glm::vec4>(const tinygltf::Model& model,
                                                int accessor_index,
...
bool GltfLoader::LoadFromFile(const std::string& filepath, sg::Scene& out_scene) {
    ...
    if (filepath.find(".glb") != std::string::npos) {
        result = loader.LoadBinaryFromFile(&model, &error_message_, &warning_message_, filepath);
    } else {
        result = loader.LoadASCIIFromFile(&model, &error_message_, &warning_message_, filepath);
    }
    ...
    // Process model data in order (images -> samplers -> textures -> materials -> meshes -> nodes -> scenes)
...
void GltfLoader::ProcessSamplers(const tinygltf::Model& model, sg::Scene& scene) {
    ...
    for (const auto& sampler : model.samplers) {
        ...
        if (sampler.minFilter >= 0) {
            sg_sampler->min_filter = static_cast<sg::Sampler::MinFilter>(sampler.minFilter);
        }
        
        // Mag filter
        if (sampler.magFilter >= 0) {
            sg_sampler->mag_filter = static_cast<sg::Sampler::MagFilter>(sampler.magFilter);
        }
...
void GltfLoader::ProcessTextures(const tinygltf::Model& model, sg::Scene& scene) {
    ...
    for (const auto& texture : model.textures) {
        ...
        // Link to image
        if (texture.source >= 0 && static_cast<size_t>(texture.source) < scene.images.size()) {
            sg_texture->image = scene.images[texture.source];
        }

        // Link to sampler
        if (texture.sampler >= 0 && static_cast<size_t>(texture.sampler) < scene.samplers.size()) {
...
void GltfLoader::ProcessMaterials(const tinygltf::Model& model, sg::Scene& scene) {
    ...
    for (const auto& material : model.materials) {
        ...
        if (material.values.find("baseColorFactor") != material.values.end()) {
            ...
            if (factor.number_array.size() == 4) {
                sg_material->pbr_metallic_roughness.base_color_factor = glm::vec4(
                    static_cast<float>(factor.number_array[0]),
        ...
        if (material.values.find("baseColorTexture") != material.values.end()) {
            int tex_index = material.values.at("baseColorTexture").json_double_value.at("index");
            if (tex_index >= 0 && static_cast<size_t>(tex_index) < scene.textures.size()) {
                sg_material->pbr_metallic_roughness.base_color_texture = scene.textures[tex_index];
            }
        }
        ...
        if (material.values.find("roughnessFactor") != material.values.end()) {
            sg_material->pbr_metallic_roughness.roughness_factor =
                static_cast<float>(material.values.at("roughnessFactor").number_value);
        }

        if (material.values.find("metallicRoughnessTexture") != material.values.end()) {
            int tex_index = material.values.at("metallicRoughnessTexture").json_double_value.at("index");
            if (tex_index >= 0 && static_cast<size_t>(tex_index) < scene.textures.size()) {
                sg_material->pbr_metallic_roughness.metallic_roughness_texture = scene.textures[tex_index];
            }
        }

        // Normal texture
        if (material.additionalValues.find("normalTexture") != material.additionalValues.end()) {
            int tex_index = material.additionalValues.at("normalTexture").json_double_value.at("index");
            if (tex_index >= 0 && static_cast<size_t>(tex_index) < scene.textures.size()) {
        ...
        if (material.additionalValues.find("occlusionTexture") != material.additionalValues.end()) {
            int tex_index = material.additionalValues.at("occlusionTexture").json_double_value.at("index");
            if (tex_index >= 0 && static_cast<size_t>(tex_index) < scene.textures.size()) {
        ...
        if (material.additionalValues.find("emissiveTexture") != material.additionalValues.end()) {
            int tex_index = material.additionalValues.at("emissiveTexture").json_double_value.at("index");
            if (tex_index >= 0 && static_cast<size_t>(tex_index) < scene.textures.size()) {
        ...
        if (material.additionalValues.find("emissiveFactor") != material.additionalValues.end()) {
            ...
            if (factor.number_array.size() == 3) {
                sg_material->emissive_factor = glm::vec3(
                    static_cast<float>(factor.number_array[0]),
                    static_cast<float>(factor.number_array[1]),
                    static_cast<float>(factor.number_array[2]));
            }
        ...
        // Double sided
        if (material.additionalValues.find("doubleSided") != material.additionalValues.end()) {
            sg_material->double_sided = material.additionalValues.at("doubleSided").bool_value;
        }

        // Alpha mode
        if (material.additionalValues.find("alphaMode") != material.additionalValues.end()) {
            const std::string& alpha_mode = material.additionalValues.at("alphaMode").string_value;
        ...
        if (material.additionalValues.find("alphaCutoff") != material.additionalValues.end()) {
            sg_material->alpha_cutoff = 
                static_cast<float>(material.additionalValues.at("alphaCutoff").number_value);
        }
    ...
    for (const auto& mesh : model.meshes) {
        ...
        for (const auto& primitive : mesh.primitives) {
            ...
            if (primitive.attributes.find("POSITION") != primitive.attributes.end()) {
                ...
                ExtractAccessorData(model, accessor_index, sg_primitive.attributes.positions);
            }

            // Normals
            if (primitive.attributes.find("NORMAL") != primitive.attributes.end()) {
                int accessor_index = primitive.attributes.at("NORMAL");
                ExtractAccessorData(model, accessor_index, sg_primitive.attributes.normals);
            }

            // Texture coordinates (set 0)
            if (primitive.attributes.find("TEXCOORD_0") != primitive.attributes.end()) {
                int accessor_index = primitive.attributes.at("TEXCOORD_0");
                ExtractAccessorData(model, accessor_index, sg_primitive.attributes.texcoords_0);
            }

            // Texture coordinates (set 1)
            if (primitive.attributes.find("TEXCOORD_1") != primitive.attributes.end()) {
                int accessor_index = primitive.attributes.at("TEXCOORD_1");
                ExtractAccessorData(model, accessor_index, sg_primitive.attributes.texcoords_1);
            }

            // Vertex colors (set 0)
            if (primitive.attributes.find("COLOR_0") != primitive.attributes.end()) {
                int accessor_index = primitive.attributes.at("COLOR_0");
                ExtractAccessorData(model, accessor_index, sg_primitive.attributes.colors_0);
            }

            // Tangents
            if (primitive.attributes.find("TANGENT") != primitive.attributes.end()) {
                int accessor_index = primitive.attributes.at("TANGENT");
                ExtractAccessorData(model, accessor_index, sg_primitive.attributes.tangents);
            }
...
void GltfLoader::ProcessNodes(const tinygltf::Model& model, sg::Scene& scene,
                               std::vector<std::shared_ptr<sg::Node>>& temp_nodes) {
    ...
    for (const auto& node : model.nodes) {
        ...
        // Link to mesh
        if (node.mesh >= 0 && static_cast<size_t>(node.mesh) < scene.meshes.size()) {
            sg_node->mesh = scene.meshes[node.mesh];
        }

        // Translation
        if (node.translation.size() == 3) {
            sg_node->translation = glm::vec3(
    ...
    for (size_t i = 0; i < model.nodes.size(); ++i) {
        ...
        for (int child_index : node.children) {
            if (child_index >= 0 && static_cast<size_t>(child_index) < temp_nodes.size()) {
...
void GltfLoader::ProcessScenes(const tinygltf::Model& model, sg::Scene& scene,
    ...
    if (scene_index >= 0 && static_cast<size_t>(scene_index) < model.scenes.size()) {
        ...
        for (int node_index : gltf_scene.nodes) {
            if (node_index >= 0 && static_cast<size_t>(node_index) < temp_nodes.size()) {
...
void GltfLoader::ExtractAccessorData<glm::vec3>(const tinygltf::Model& model,
    ...
    // Check if buffer data is loaded (external URIs are loaded by tinygltf automatically)
    ...
    if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT) {
        ...
        for (size_t i = 0; i < accessor.count; ++i) {
            out_data[i] = glm::vec3(float_ptr[i * stride + 0],
                                    float_ptr[i * stride + 1],
...
void GltfLoader::ExtractAccessorData<glm::vec2>(const tinygltf::Model& model,
    ...
    if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT) {
        ...
        for (size_t i = 0; i < accessor.count; ++i) {
            out_data[i] = glm::vec2(float_ptr[i * stride + 0], float_ptr[i * stride + 1]);
        }
    }
}
...
void GltfLoader::ExtractAccessorData<glm::vec4>(const tinygltf::Model& model,
    ...
    if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT) {
        ...
        for (size_t i = 0; i < accessor.count; ++i) {
            out_data[i] = glm::vec4(float_ptr[i * stride + 0],
                                    float_ptr[i * stride + 1],
                                    float_ptr[i * stride + 2],
                                    float_ptr[i * stride + 3]);
        }
    }
}
