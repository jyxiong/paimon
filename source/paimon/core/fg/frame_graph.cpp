...
void FrameGraph::exportExecutionOrderToDot(const std::string &filename) const {
  ...
  for (const auto &pass : m_pass_nodes) {
    ...
    std::string label = std::to_string(executionIndex++) + ": " + std::string{pass.getName()};
    file << "  exec_" << pass.getId() 
         << " [label=\"" << label << "\"];\n";
  ...
  for (const auto &resource : m_resource_nodes) {
    ...
    if (resource.getProducer()) {
      file << "  exec_" << resource.getProducer()->getId() 
           << " -> res_" << resource.getId() 
    ...
    if (resource.getLastConsumer()) {
      file << "  res_" << resource.getId() 
           << " -> exec_" << resource.getLastConsumer()->getId() 
