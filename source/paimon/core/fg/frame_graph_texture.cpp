...
void FrameGraphTexture::preRead(
  void* context, const Descriptor& desc, uint32_t flags
) {
  // auto* rc = static_cast<RenderContext*>(context);
  // rc->barrierTexture(
  //   *m_texture, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT
  // );
}

void FrameGraphTexture::preWrite(
  void* context, const Descriptor& desc, uint32_t flags
) {
  // auto* rc = static_cast<RenderContext*>(context);
  // rc->barrierTexture(
  //   *m_texture, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_UPDATE_BARRIER_BIT
  // );
}