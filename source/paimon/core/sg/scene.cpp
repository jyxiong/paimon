...
void Scene::AddMesh(std::shared_ptr<Mesh> mesh) { meshes.push_back(mesh); }
...
void Scene::AddImage(std::shared_ptr<Image> image) { images.push_back(image); }
...
size_t Scene::GetNodeCount() const {
  ...
  Traverse([&](const Node * /*node*/) { ++count; });
